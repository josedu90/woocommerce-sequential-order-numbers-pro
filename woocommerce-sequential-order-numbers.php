 ...
 * Plugin URI: http://www.woocommerce.com/products/sequential-order-numbers-pro/
...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.9.0', __( 'WooCommerce Sequential Order Numbers Pro', 'woocommerce-sequential-order-numbers-pro' ), __FILE__, 'init_woocommerce_sequential_order_numbers_pro', array(
...
class WC_Seq_Order_Number_Pro extends SV_WC_Plugin {
	...
	public function __construct() {
		...
		// Drop this action when legacy REST API v1-v3 are no longer supported {BR 2017-04-17}
		add_action( 'woocommerce_api_create_order', array( $this, 'set_sequential_order_number' ), 10, 1 );
		// WC REST API v2 support (WC 3.0+)
		add_action( 'woocommerce_rest_insert_shop_order_object', array( $this, 'set_sequential_order_number' ), 10, 1 );

		// WC REST API v1 support (WC 2.6+)
		// see docblock as to why this is handled separately
		add_action( 'woocommerce_rest_insert_shop_order', array( $this, 'handle_rest_api_v1_orders' ), 10, 1 );
		...
		// keep this early before WC tries to hook into this filter to "sanitize" the order ID, stripping any letters
	...
	public function find_order_by_order_number( $order_number ) {
		...
		if ( empty( $order ) || '' !== SV_WC_Order_Compatibility::get_meta( $order, 'order_number_formatted' ) ) {
		...
		return SV_WC_Order_Compatibility::get_prop( $order, 'id' );
	}


	/**
	 * Handles orders created via the /wp-json/wc/v1/ (WC 2.6+) endpoint.
	 *
	 * This check is needed because we don't get a WC_Order here, but a WP_Post instead, so we need to
	 *  get the order and pass it to $this->set_sequential_order_number().
	 *
	 * @since 1.11.1
	 *
	 * @param \WP_Post $post the WP_Post object for the created order.
	 */
	public function handle_rest_api_v1_orders( $post ) {

		// double-check in case what's passed into this hook changes in the future
		if ( $post instanceof WC_Order ) {
			$this->set_sequential_order_number( $post );
			return;
		}

		// otherwise set the order number if we've got an order
		if ( $post instanceof WP_Post && $order = wc_get_order( $post->ID ) ) {
			$this->set_sequential_order_number( $order );
		}
	}
	...
	public function set_sequential_order_number( $post_id, $post = array() ) {
		...
		if ( is_array( $post ) || is_null( $post ) || ( 'shop_order' === $post->post_type && 'auto-draft' !== $post->post_status ) ) {
			...
			$post_id      = $post_id instanceof WC_Order ? SV_WC_Order_Compatibility::get_prop( $post_id, 'id' ) : $post_id;
			$order        = $post_id instanceof WC_Order ? $post_id : wc_get_order( $post_id );
			$order_number = SV_WC_Order_Compatibility::get_meta( $order, '_order_number' );
			...
			if ( empty( $order_number ) ) {
				...
				if ( $this->skip_free_orders() && $this->is_free_order( $post_id ) ) {
					...
					if ( $this->generate_sequential_order_number( $post_id, '_order_number_free', $this->get_free_order_number_start(), $this->get_free_order_number_prefix() ) ) {
						...
						SV_WC_Order_Compatibility::update_meta_data( $order, '_order_number', -1 );
					}
	...
	private function generate_sequential_order_number( $post_id, $order_number_meta_name, $order_number_start, $order_number_prefix = '', $order_number_suffix = '', $order_number_length = 1 ) {
		...
		for ( $i = 0; $i < 3 && ! $success; $i++ ) {
			...
			// a db option, this is useful for sites with huge databases
				...
				// HyperDB compat: make sure that the following couple statements
				// run on the same db server.
			...
			if ( null !== $order_number ) {
				...
				SV_WC_Order_Compatibility::update_meta_data( $order, '_order_number_formatted', $this->format_order_number( $order_number, $order_number_prefix, $order_number_suffix, $order_number_length, $post_id ) );
				...
				if ( $this->is_performance_mode_enabled() ) {
					SV_WC_Order_Compatibility::update_meta_data( $order, $order_number_meta_name, $order_number );
				}
				...
				SV_WC_Order_Compatibility::update_meta_data( $order, '_order_number_meta', $order_number_meta );
			}
		}
	...
	public function get_order_number( $order_number, $order ) {
		...
		$order_number_formatted = SV_WC_Order_Compatibility::get_meta( $order, '_order_number_formatted' );
		...
		$post_status = get_post_status( SV_WC_Order_Compatibility::get_prop( $order, 'id' ) );
		...
		if ( 'auto-draft' === $post_status ) {
			...
			return $this->format_order_number( $order_number, $this->get_order_number_prefix(), $this->get_order_number_suffix(), $this->get_order_number_length(), SV_WC_Order_Compatibility::get_prop( $order, 'id' ) ) . ' (' . __( 'Draft', 'woocommerce-sequential-order-numbers-pro' ) . ')';
		}
	...
	public function admin_settings_js() {
		...
		function formatOrderNumber( orderNumber, orderNumberPrefix, orderNumberSuffix ) {
			...
			if ( formatted.indexOf( '{N}' )    > -1) formatted = formatted.replace( /{N}/g,    d.getMinutes() );
			if ( formatted.indexOf( '{S}' )    > -1) formatted = formatted.replace( /{S}/g,    d.getSeconds() );
	...
	public function subscriptions_set_sequential_order_number( $renewal_order ) {
		...
		$order_post = get_post( SV_WC_Order_Compatibility::get_prop( $renewal_order, 'id' ) );
	 ...
	 * http://docs.woocommerce.com/document/woocommerce-{dasherized-plugin-id}/
	 *
	 * @since 1.6
				...
				if ( is_array( $order_ids ) ) {
					...
					foreach( $order_ids as $order_id ) {
						...
						if ( '' === $order_number && '' === $order_number_formatted ) {
							...
							SV_WC_Order_Compatibility::update_meta_data( $order, '_order_number', $order_id );
							SV_WC_Order_Compatibility::update_meta_data( $order, '_order_number_formatted', $order_id );
						} elseif ( '' === $order_number_formatted ) {
							// an order from the free sequential order number plugin, add the _order_number_formatted field
							SV_WC_Order_Compatibility::update_meta_data( $order, '_order_number_formatted', $order_number );
						}
					}
				}
